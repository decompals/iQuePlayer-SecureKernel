...
s32 verify_crlbundle(BbCrlBundle* bundle, u32 type, u32 requiredVersion) {
    ...
    if (strcmp((const char*)bundle->head->issuer, "Root") == 0) {
        ret = rsa_verify_signature(dataBlocks, ARRAY_COUNT(dataBlocks), rootRSAPublicKey, rootRSAExponent, 1,
                                   (u32*)&bundle->head->signature);
    } else {
...
s32 verify_all_crlbundles(BbCrlBundle* carl, s32 requiredCarlVersion, // force formatting
...
s32 check_ticket_bundle_revocations(BbTicketBundle* ticketBundle, BbAppLaunchCrls* crls) {
    ...
    ret = verify_all_crlbundles(&crls->carl, ticketBundle->ticket->cmd.head.caCrlVersion, // force formatting
