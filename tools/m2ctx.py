...
def import_c_file(in_file):
    ...
    cpp_command = ["gcc", "-E", "-P", "-Iinclude", "-Iinclude/PR", "-Isrc", "-undef", "-D__sgi", "-DBBPLAYER", "-D_LANGUAGE_C",
                   "-DNON_MATCHING", "-D_Static_assert(x, y)=", "-D__asm__(x)=", "-D_MIPS_SZLONG=32", "-D__attribute__(x)=", in_file]
    try:
